...
struct NLaterationSolver {
	...
	func solveEmitterPosition() -> [Float]? {
		...
		if(getReceiverCount() < 4) else {return nil}
		
		return [0.0, 0.0, 0.0]
    }

    func solveEmitterPosition(receivers: [String: Receiver], emitter: Emitter) -> [Float]? {
    guard receivers.count >= 4 else { return nil }
